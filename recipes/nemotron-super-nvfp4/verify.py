#!/usr/bin/env python3
"""CPU-only checkpoint preflight. All files are authenticated at setup; serving metadata is authenticated on every start."""
...
def verify(root: Path, manifest: dict, full_hash: bool = False) -> dict:
    ...
    for number, item in enumerate(manifest["files"], start=1):
        ...
        if digest and (full_hash or item["size"] <= 32 * 1024 * 1024):
            with path.open("rb") as source:
    ...
    if config.get("architectures") != manifest["architectures"]:
        raise ValueError("checkpoint architecture does not match the pinned Nemotron recipe")
    ...
    for name in ("tokenizer_config.json", "generation_config.json", "hf_quant_config.json",
...
def main() -> None:
    ...
    try:
        ...
        parser.exit(1, f"Nemotron checkpoint preflight failed: {exc}\nRun ./spark-serve pull nemotron-super --node head or --node worker to prepare the selected Spark.\n")
